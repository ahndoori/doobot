...
사용자의 음성을 인식하고 화면을 분석하여 매크로를 실행하는 반자동 AI 에이전트
...
* **주요 스택:** `Vosk`, `Whisper`
* **역할:** 로컬 환경에서 가볍고 빠르게 유저의 음성 스트림을 텍스트로 디코딩. 레이턴시 최소화를 위해 온디바이스(On-device) 엔진 채택

### 2. Brain & LLM (명령 판독 및 라우팅)
* **주요 스택:** `Ollama` (`Llama-3` / `EEVE-Korean`), `Pydantic`
...
* **주요 스택:** `EasyOCR`, `Tesseract`, `OpenCV`, `Pillow (PIL)`
...
* **주요 스택:** `PyAutoGUI`, `Pure-Python-ADB`, `Kokoro-82M`
